/* =============================================================================
   موتور API سامانه پیام داروساز — حالت IRC و حالت ژنریک (بدون کپچا، با توکن لاگین)
   -----------------------------------------------------------------------------
   کشف کلیدی: درخواست‌های API با هدر  Authorization: Bearer <توکن لاگین>  نیازی
   به کپچا ندارند. توکن از localStorage کلید 'ls.authorizationData' خوانده می‌شود.
   endpoint جستجو:  /api/v3/frontOffice/products?keyword=...&page=1&pageSize=10
   فیلدهای مفید هر آیتم: id, isGeneric, code5, irc, genericCode5, fullNameFa,
   fullNameEn, payamCode, latestPrice

   بارگذاری در Console سایت (تب لاگین‌شده payamdaroosaz.com):
     fetch('https://raw.githubusercontent.com/mohammadzdbdi-maker/INTRACTIONS/arena/01a0c5ce-intractions/PayamD-Files/tools/payamdaroosaz-irc-api.js').then(r=>r.text()).then(t=>eval(t))

   فرمان‌ها:
     __psaRunIRC(limit?)   شروع/ادامهٔ حلقهٔ 34,731 کد IRC (لیست از گیت‌هاب)
     __psaRunGen(limit?)   حلقهٔ سریع 1,704 کد ژنریک جامانده (تعبیه‌شده)
     __psaStatus()         گزارش پیشرفت
     __psaStop()           توقف ایمن (نتایج تا لحظهٔ توقف ذخیره شده‌اند)
     __psaExportIRC()      دانلود CSV نتایج IRC
     __psaExportGen()      دانلود CSV نتایج ژنریک
     __psaResetIRC() / __psaResetGen()   پاک‌کردن حافظهٔ پیشرفت
   ============================================================================= */
(function () {
  'use strict';
  ...
  var K_IRC = '__psa_irc_v1';
  var K_GEN = '__psa_gen_v1';
  var CONC = 3;          // تعداد درخواست هم‌زمان
  var GAP_MS = 350;      // فاصلهٔ بین شروع درخواست‌ها در هر کارگر
  var BACKOFF_MS = 45000;
  var MAX_STRIKES = 4;

  window.__PSA = window.__PSA || {};
  var PSA = window.__PSA;
  PSA.ircs = null;       // پس از دانلود لیست پر می‌شود
  PSA.running = false;
  PSA.stopReq = false;
  ...
  function isWafText(t) {
    return /آروان|arvan|challenge|دسترسی شما مسدود|captcha-cloud|<html/i.test(t || '');
  }

  // یک درخواست جستجو؛ خروجی: {status, json|null, text}
  function search(kw) {
    var tok = authTok();
  ...
  function runQueue(list, key, picker, opts) {
    if (PSA.running) { log('یک حلقه هم‌اکنون در حال اجراست؛ اول __psaStop()'); return; }
    var state = loadState(key);
    var todo = list.filter(function (k) { return !state[k]; });
    var limit = (opts && opts.limit) || todo.length;
    todo = todo.slice(0, limit);
    if (!todo.length) { log('همهٔ موارد قبلاً پردازش شده‌اند. خروجی: __psaExport…'); return; }
    PSA.running = true; PSA.stopReq = false;
    PSA.stats = { done: 0, found: 0, notfound: 0, multi: 0, errs: 0, strikes: 0 };
    var idx = 0, saveTick = 0, stopped = null;

    function finish(msg) {
      PSA.running = false;
      saveState(key, state);
      log('پایان/توقف: ' + (msg || '') + ' | پردازش‌شده این دور: ' + PSA.stats.done +
          ' یافت:' + PSA.stats.found + ' نیافت:' + PSA.stats.notfound + ' چندنتیجه:' + PSA.stats.multi + ' خطا:' + PSA.stats.errs +
          ' | ذخیره‌شده کل: ' + Object.keys(state).length + ' از ' + list.length);
      log('ادامه: همان فرمان را دوباره بزنید. خروجی: __psaExport…');
    }
        ...
        return new Promise(function (res) { setTimeout(res, GAP_MS); })
          ...
          .then(function (r) {
            if (r.status === 200 && r.json) {
              ...
              stopped = 'توکن لاگین معتبر نیست (401/403)؛ دوباره در سایت لاگین کنید و ادامه دهید';
            } else if (r.status === 400) {
              PSA.stats.strikes = MAX_STRIKES;
              stopped = 'خطای 400 (احتمالاً کپچا/216)؛ یک جستجوی دستی انجام دهید و ادامه دهید';
            ...
            if (++saveTick % 25 === 0) saveState(key, state);
            return next();
          })
          .then(function () { /* ادامه در next */ });
      })();
    }

    var ws = [];
    for (var w = 0; w < CONC; w++) ws.push(worker());
    Promise.all(ws).then(function () { finish(stopped || 'تکمیل شد'); });
  }
  ...
  window.__psaRunIRC = function (limit) {
    ensureList(function (list) { runQueue(list, K_IRC, pickIRC, { limit: limit }); });
  };
  window.__psaExportIRC = function () {
    ensureList(function (list) {
      var st = loadState(K_IRC);
  ...
  window.__psaResetIRC = function () { localStorage.removeItem(K_IRC); log('حافظهٔ IRC پاک شد'); };

  // ---------- حالت ژنریک (1704 جامانده) ----------
  window.__psaRunGen = function (limit) { runQueue(GEN1704, K_GEN, pickGen, { limit: limit, pad: true }); };
  window.__psaExportGen = function () {
    var st = loadState(K_GEN);
  ...
  window.__psaResetGen = function () { localStorage.removeItem(K_GEN); log('حافظهٔ ژنریک پاک شد'); };

  // ---------- مشترک ----------
  window.__psaStop = function () { PSA.stopReq = true; log('درخواست توقف داده شد؛ چند ثانیه…'); };
  window.__psaStatus = function () {
    var si = Object.keys(loadState(K_IRC)).length;
    var sg = Object.keys(loadState(K_GEN)).length;
    log('IRC ذخیره‌شده: ' + si + (PSA.ircs ? '/' + PSA.ircs.length : '/?') +
        ' | ژنریک ذخیره‌شده: ' + sg + '/' + GEN1704.length +
        ' | در حال اجرا: ' + (PSA.running ? 'بله ' + JSON.stringify(PSA.stats) : 'خیر'));
  };

  log('موتور API آماده است. فرمان‌ها: __psaRunIRC() | __psaRunGen() | __psaStatus() | __psaStop() | __psaExportIRC() | __psaExportGen()');
  log('نکته: تب را باز و نمایان نگه دارید؛ نتایج پس از هر ۲۵ مورد در localStorage ذخیره می‌شوند (مقاوم در برابر رفرش).');
  ensureList(function () {});
})();
