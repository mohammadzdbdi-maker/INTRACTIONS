...
(function () {
  ...
  var RAW_HERB = 'https://raw.githubusercontent.com/mohammadzdbdi-maker/INTRACTIONS/arena/01a0c5ce-intractions/PayamD-Files/data/teamtech_herb_ircs.json';
  var RAW_HERBNAME = 'https://raw.githubusercontent.com/mohammadzdbdi-maker/INTRACTIONS/arena/01a0c5ce-intractions/PayamD-Files/data/herb_name_search.json';
  ...
  function db() {
    ...
    DBP = new Promise(function (res, rej) {
      var rq = indexedDB.open('psa_db', 9);
      rq.onupgradeneeded = function () {
        ...
        if (!d.objectStoreNames.contains('supph')) d.createObjectStore('supph', { keyPath: 'k' });
        if (!d.objectStoreNames.contains('suppnameh')) d.createObjectStore('suppnameh', { keyPath: 'k' });
      };
  ...
  /* ---------------- حالت گیاهی: IRC مستقیم + جستجوی نام ---------------- */
  function ensureHerb(cb) {
    if (PSA.herb) { cb(PSA.herb); return; }
    log('در حال دانلود لیست IRC گیاهی…');
    fetch(RAW_HERB).then(function (r) { return r.json(); }).then(function (j) {
      PSA.herb = j; log('HERB LIST READY: ' + j.length + ' کد IRC گیاهی. حالا: __psaRunSuppH()');
      cb(j);
    }).catch(function (e) { log('خطا در دانلود لیست گیاهی: ' + e); });
  }
  window.__psaRunSuppH = function (limit) { ensureHerb(function (list) { runQueue(list, 'supph', pickSupp, { limit: limit }); }); };
  window.__psaExportSuppH = function () {
    ensureHerb(function (list) {
      stateMap('supph').then(function (st) {
        var rows = [['کد IRC', 'کد پیام', 'نام فارسی', 'نام انگلیسی', 'تعداد نتایج', 'وضعیت', 'قیمت واحد فعلی (ریال)', 'تاریخ به‌روزرسانی (میلادی)', 'همه کدهای پیام']];
        list.forEach(function (k) {
          var r = st[k];
          if (!r) { rows.push([k, '', '', '', '', 'pending', '', '', '']); return; }
        ...
        csvDownload('payam_supph_results.csv', rows);
      });
    });
  };
  function ensurePairsH(cb) {
    if (PSA.pairsH) { cb(PSA.pairsH); return; }
    log('در حال دانلود لیست نام گیاهی…');
    fetch(RAW_HERBNAME).then(function (r) { return r.json(); }).then(function (j) {
      PSA.pairsH = j; log('HERBNAME LIST READY: ' + j.length + ' ردیف. حالا: __psaRunSuppNameH()');
      cb(j);
    }).catch(function (e) { log('خطا در دانلود لیست نام گیاهی: ' + e); });
  }
  window.__psaRunSuppNameH = function (limit) {
    ensurePairsH(function (pairs) {
      var keys = pairs.map(function (x) { return x[0]; });
      PSA.pairMapH = {}; pairs.forEach(function (x) { PSA.pairMapH[x[0]] = x[1]; });
      runQueue(keys, 'suppnameh', function (json, kw) { return pickCands(json, PSA.pairMapH[kw] || kw); }, { limit: limit, kwOf: function (k) { return PSA.pairMapH[k] || k; } });
    });
  };
  window.__psaExportSuppNameH = function () {
    ensurePairsH(function (pairs) {
      stateMap('suppnameh').then(function (st) {
        var rows = [['کد IRC', 'کلمه جستجو', 'تعداد نتایج', 'کد پیام کاندیدا', 'نام فارسی کاندیدا', 'نام انگلیسی کاندیدا', 'کد ژنریک کاندیدا', 'IRC کاندیدا', 'قیمت کاندیدا (ریال)', 'تاریخ قیمت (میلادی)']];
        pairs.forEach(function (x) {
          var irc = x[0], kw = x[1], r = st[irc];
          if (!r) { rows.push([irc, kw, 'pending', '', '', '', '', '', '', '']); return; }
        ...
        csvDownload('payam_suppnameh_results.csv', rows);
      });
    });
  };
  ...
  log('موتور API نسخه ۱۱ (IndexedDB) آماده است. فرمان‌ها: __psaRunIRC() | __psaRunSupp() | __psaRunGen() | __psaRunPrice() | __psaRunSuppName4() | __psaProbeAll2() | __psaRunCatalog(1,100) | __psaRunCatalogStuck() | __psaRunSuppH() | __psaExportSuppH() | __psaRunSuppNameH() | __psaExportSuppNameH() | __psaExportCatalog() | __psaStatus() | __psaStop()');
  migrate();
  ensureList(function () {});
})();
